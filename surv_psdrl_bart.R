#' @title psDR-learner with NFT-BART


surv_psdrl_bart <- function(X, Y, W, D, t0, W.hat = NULL, cen.fit = "nftbart",
  ...
  D.t0 <- D
  D.t0[D == 1 & Y > t0] <- 0
  D.t0 <- D.t0[(D == 1 | Y > t0) & W == 1]
  W.t0 <- W[(D == 1 | Y > t0) & W == 1]
  X.t0 <- X[(D == 1 | Y > t0) & W == 1,, drop = FALSE]
  Tgrf0.t0 <- Tgrf0[(D == 1 | Y > t0) & W == 1]
  Tgrf1.t0 <- Tgrf1[(D == 1 | Y > t0) & W == 1]
  sample.weights.t0 <- 1 / C.hat[(D == 1 | Y > t0) & W == 1]
  W.hat.t0 <- W.hat[(D == 1 | Y > t0) & W == 1]

  Z <- (D.t0 - Tgrf0.t0) * (W.t0 - W.hat.t0) /
    ((1 - W.hat.t0) * W.t0)
  ...
  ret <- list(tau.fit = tau.fit,
              tau.hat = tau.hat)
  class(ret) <- "surv_psdrl_bart"
  ret
}

predict.surv_psdrl_bart <- function(object,
                                  newdata = NULL,
                                  ...) {
  if (!is.null(newdata)) {
